...
<template>
    <div>
        <Dialog header="Processor Type" v-model:visible="displayModal" @hide="closeModal" :breakpoints="{ '960px': '75vw', '640px': '90vw' }"
            :style="{ width: '40vw' }" :modal="true">
            <div class="">
                <div class="space-y-4">
                    <div class="">
                        <label for="type" class="block text-sm font-medium text-gray-700">Type</label>
                        <InputText type="type" v-model="type" id="type"
            ...
            <template #footer>
                ...
                <Button label="Add Item" icon="pi pi-check" @click="submitProcessorType" class="blue-bg text-white"
                    autofocus />
            </template>
        </Dialog>
        <!-- <Toast /> -->
...
watch(modal, value => {
    if (modal.value == 'processortype') displayModal.value = true;
})
...
async function submitProcessorType() {
    const payload = {
        id: 0,
        type: type.value,
    ...
    return await measurementStore.saveProcessorType(payload)
}
