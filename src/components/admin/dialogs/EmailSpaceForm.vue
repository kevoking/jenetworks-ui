...
<template>
    <div>
        <Dialog header="Email Space" v-model:visible="displayModal" @hide="closeModal" :breakpoints="{ '960px': '75vw', '640px': '90vw' }"
            :style="{ width: '40vw' }" :modal="true">
            <div class="">
                <div class="space-y-4">
                    <div class="">
                        <label for="size" class="block text-sm font-medium text-gray-700">Total Space</label>
                        <InputText type="text" v-model="totalSpace" id="size"
            ...
            <template #footer>
                ...
                <Button label="Add Item" icon="pi pi-check" @click="submitEmailSpace" class="blue-bg text-white"
                    autofocus />
            </template>
        </Dialog>
        <!-- <Toast /> -->
...
watch(modal, value => {
    if (modal.value == 'emailspace') displayModal.value = true;
...
async function submitEmailSpace() {
    const payload = {
        id: 0,
    ...
    await measurementStore.saveEmailSpace(payload)
}
